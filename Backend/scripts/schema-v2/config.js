...
function getSeedConfig() {
  const config = {
    ...
    borrowerCount: integerFromEnv('SEED_BORROWER_COUNT', DEFAULTS.borrowerCount),
    listingCount: integerFromEnv('SEED_LISTING_COUNT', DEFAULTS.listingCount),
    applicationCount: integerFromEnv('SEED_APPLICATION_COUNT', DEFAULTS.applicationCount),
    loanCount: integerFromEnv('SEED_LOAN_COUNT', DEFAULTS.loanCount),
    defaultPassword: process.env.SEED_DEFAULT_PASSWORD || DEFAULTS.defaultPassword,
  };

  if (!config.randomSeed.trim()) throw new Error('SEED_RANDOM_SEED cannot be empty.');
  if (!/^[a-zA-Z0-9_-]+$/.test(config.batchId)) {
    throw new Error('SEED_BATCH_ID may contain only letters, numbers, underscores, and hyphens.');
  }
  ...
  if (config.listingCount > 0 && config.lenderCount === 0) {
    throw new Error('SEED_LENDER_COUNT must be positive when listings are requested.');
  }
  if (config.applicationCount > 0 && (config.listingCount === 0 || config.borrowerCount === 0)) {

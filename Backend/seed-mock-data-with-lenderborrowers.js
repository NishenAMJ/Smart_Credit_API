 ...
 * @property {FirebaseFirestore.Timestamp} createdAt
 * @property {FirebaseFirestore.Timestamp} paidAt
 * @property {FirebaseFirestore.Timestamp} updatedAt
 * @property {FirebaseFirestore.Timestamp=} verifiedAt
...
async function clearExistingSeedData(db) {
  const collections = [
    USERS_COLLECTION,
    ADS_COLLECTION,
    REQUESTS_COLLECTION,
    LOANS_COLLECTION,
    LENDER_BORROWERS_COLLECTION,
    TRANSACTIONS_COLLECTION,
    DISPUTES_COLLECTION,
    LENDER_SETTINGS_COLLECTION,
    LENDER_NOTIFICATIONS_COLLECTION
  ];
...
async function createUsers(db, now, rng) {
  ...
  const pendingUsers = ['lender_002', 'borrower_008'];
  ...
  const suspendedUsers = [
    ['lender_005', 'Repeated document mismatches during compliance review'],
    ['borrower_009', 'Account temporarily suspended after repeated payment disputes'],
  ];
  ...
  const rejectedUsers = [
    ['lender_004', 'Uploaded business registration document was unreadable'],
    ['borrower_010', 'National ID details could not be verified'],
  ];
...
function createLoanDistribution() {
  // Lightweight seed distribution to keep Firestore write and storage quota low
  // while still giving the admin module enough variety to review.
  const activeCounts = [2, 2, 1, 1, 0];
  const completedExtras = [1, 0, 0, 0, 0];
...
function createAdsAndLoans(db, now, rng, lenders, borrowers) {
  ...
  distribution.forEach((entry, distributionIndex) => {
    ...
    for (let loanOffset = 0; loanOffset < entry.totalLoans; loanOffset += 1) {
      ...
      const tenureMonths = pick(rng, [6, 9, 12]);
        ...
        paymentsForInstallment.forEach((paymentWrite) => {
          ...
          transactionWrites.push({
            ...
            data: {
              ...
              loanId: loanRef.id,
              installmentId,
              paymentId: paymentWrite.data.paymentId,
              lenderId: lender.uid,
              borrowerId: borrower.uid,
              lenderName: lender.fullName,
              ...
              platformFee: roundCurrency(Math.max(100, paymentWrite.data.amount * 0.02)),
              paymentType: paymentWrite.data.paymentType,
              type: 'repayment',
              status: 'completed',
              verifiedByLender: true,
              createdAt: paymentWrite.data.paidAt,
              paidAt: paymentWrite.data.paidAt,
              updatedAt: paymentWrite.data.verifiedAt || paymentWrite.data.paidAt,
              verifiedAt: paymentWrite.data.verifiedAt || paymentWrite.data.paidAt,
              receiptURL: paymentWrite.data.receiptURL,
              qrScanData: paymentWrite.data.qrScanData,
              note: 'Seeded repayment transaction'
            }
          });
          transactionsCount += 1;
        });
    ...
    if (entry.totalLoans === 0) {
      ...
      adWrites.push({
        ...
        data: {
          ...
          minTenureMonths: 6,
          maxTenureMonths: pick(rng, [12, 18]),
          preferredPurposes: pickPurposes(rng),
          location: pick(rng, LOCATIONS),
          status: 'active',
  ...
  const sampleLoans = loanWrites.slice(0, 2);
  sampleLoans.forEach((loanWrite, index) => {
    const lender = lenders.get(loanWrite.data.lenderId);
    const borrower = borrowers.get(loanWrite.data.borrowerId);
    if (!lender || !borrower) {
      return;
    }

    const transactionRef = db.collection(TRANSACTIONS_COLLECTION).doc();
    const createdAt = ts(addDays(now, -(index + 2)));
    const updatedAt = ts(addDays(now, -(index + 1)));
    ...
    const transactionDoc = {
      ...
      borrowerName: borrower.fullName,
      borrowerEmail: borrower.email,
      amount: roundCurrency(randomInt(rng, 5000, 30000)),
      platformFee: roundCurrency(randomInt(rng, 100, 1200)),
      paymentType: index === 0 ? 'manual' : 'qr',
      type: 'repayment',
      ...
      updatedAt,
      note: isFailed
        ? 'Seeded failed transaction for admin filtering'
        : 'Seeded pending transaction awaiting lender verification'
    };

    transactionWrites.push({ ref: transactionRef, data: transactionDoc });
    transactionsCount += 1;
  });
...
async function main() {
  ...
  if (options.reset) {
    console.log('Reset mode enabled. Removing existing seeded collections first...');

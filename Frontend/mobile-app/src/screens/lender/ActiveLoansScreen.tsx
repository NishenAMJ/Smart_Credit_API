...
export default function ActiveLoansScreen({ navigation }: any) {
  ...
  const getLoanStatusStyle = (loanStatus: string) => {
    switch ((loanStatus ?? 'active').toLowerCase()) {
      case 'completed': return { bg: '#ECFDF5', color: COLORS.success, label: 'COMPLETED' };
      case 'overdue':   return { bg: '#FEF2F2', color: COLORS.danger,  label: 'OVERDUE'   };
      default:          return { bg: '#EBF4FF', color: COLORS.primary, label: 'ACTIVE'    };
    }
  };

  const daysUntilDue = (nextDueDate: string | null | undefined): number | null => {
  ...
  return (
    <SafeAreaView style={commonStyles.safe}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
          ...
          <View style={[commonStyles.card, { marginBottom: 4 }]}>
            <View style={commonStyles.rowSpaceBetween}>
              <View style={{ alignItems: 'center' }}>
                <Text style={styles.sumNum}>{summary.loansWithActivity ?? allLoans.length}</Text>
              ...
              <View style={{ alignItems: 'center' }}>
                <Text style={[styles.sumNum, { color: summary.overdueInstallments > 0 ? COLORS.danger : COLORS.textSecondary }]}>
        ...
        {filtered.length === 0 ? (
          <AlertBanner type="info" title="No Loans" message={`No ${filter === 'all' ? '' : filter} loans found`} />
        ) : (
          filtered.map((loan: any) => {
            ...
            return (
              <TouchableOpacity
                key={loan.loanId ?? loan.transactionId}
                style={commonStyles.card}
                onPress={() => navigation.push('LoanDetails', { loanId: loan.loanId })}
                activeOpacity={0.8}
              >
                {/* Header row */}
                <View style={commonStyles.rowSpaceBetween}>
                  <View style={{ flex: 1 }}>
                    <Text style={commonStyles.sectionTitle}>{loan.borrowerName ?? 'Unknown'}</Text>
                ...
                <View style={[commonStyles.rowSpaceBetween, { marginTop: 12 }]}>
                  <View>
                    ...
                    <Text style={commonStyles.textPrimary}>LKR {principal.toLocaleString()}</Text>
                  </View>
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={commonStyles.textSecondary}>Outstanding</Text>
                    <Text style={[commonStyles.textPrimary, { color: remaining > 0 ? COLORS.warning : COLORS.success }]}>
                ...
                {total > 0 && (
                  <View style={{ marginTop: 12 }}>
                    <View style={commonStyles.rowSpaceBetween}>
                      <Text style={commonStyles.textSecondary}>{paid}/{total} installments</Text>
                ...
                {/* Next due / days */}
                {days !== null && (loan.loanStatus ?? 'active') !== 'completed' && (
                  <View style={[commonStyles.row, { marginTop: 8 }]}>
                    <Feather
                      name="clock"
                      size={13}
                      color={days < 0 ? COLORS.danger : days <= 7 ? COLORS.warning : COLORS.textSecondary}
                    />
                    <Text style={[commonStyles.textSecondary, {
                      marginLeft: 4,
                      color: days < 0 ? COLORS.danger : days <= 7 ? COLORS.warning : COLORS.textSecondary,
                    }]}>
                      {days < 0 ? `${Math.abs(days)} days overdue` : days === 0 ? 'Due today' : `${days} days until next payment`}
                    </Text>
                  </View>
                )}
              </TouchableOpacity>

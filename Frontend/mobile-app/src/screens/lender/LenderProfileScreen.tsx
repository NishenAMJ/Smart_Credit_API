...
export default function LenderProfileScreen({ navigation }: any) {
  ...
  const PROFILE_DATA = {
    ...
    totalLoaned:
      profile?.totalLoaned != null
        ? `LKR ${Number(profile.totalLoaned).toLocaleString()}`
        : "LKR --",
    totalReturned:
      profile?.totalReturned != null
        ? `LKR ${Number(profile.totalReturned).toLocaleString()}`
        : "LKR --",
  };
  ...
  const handleSavePassword = async () => {
    ...
    try {
      ...
      await LenderProfileService.changePassword({
        currentPassword,
        newPassword,
      });
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: () => {
          try {
            ...
            Alert.alert(
              "Error",
              "Could not logout properly. Please restart the app.",
            );
          }
        },
      },
  ...
  const getPasswordStrength = (pwd: string) => {
    ...
    if (pwd.length < 6)
      return { label: "Weak", color: COLORS.danger, width: "25%" };
    if (pwd.length < 8)
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.expandedSection}>
            ...
            <View style={styles.tipBox}>
              ...
              <Text
                style={[
                  styles.tipText,
                  newPassword.length >= 8 && styles.tipDone,
                ]}
              >
              ...
              <Text
                style={[
                  styles.tipText,
                  /[A-Z]/.test(newPassword) && styles.tipDone,
                ]}
              >
              ...
              <Text
                style={[
                  styles.tipText,
                  /[0-9]/.test(newPassword) && styles.tipDone,
                ]}
              >
        ...
        <View style={styles.settingsList}>
          <View style={[commonStyles.rowSpaceBetween, styles.settingItem]}>
            <View style={commonStyles.row}>
              <View
                style={[styles.settingIcon, { backgroundColor: "#EBF4FF" }]}
              >

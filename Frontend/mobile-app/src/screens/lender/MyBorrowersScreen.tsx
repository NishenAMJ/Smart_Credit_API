...
export default function MyBorrowersScreen({ navigation }: any) {
  ...
  const renderCard = ({ item }: any) => {
    ...
    const activeLoans =
      item.activeLoansCount ?? item.activeLoans ?? item.loans ?? 0;
    const totalBorrowed =
      item.totalBorrowedAmount ?? item.totalBorrowed ?? item.total ?? null;
    const loanCount = item.loanCount ?? item.loans ?? 0;

    // Navigate to parent stack navigator for BorrowerDetail screen
    const handlePress = () => {
      navigation.getParent()?.navigate("BorrowerDetail", { borrower: item });
    };
          ...
          {item.kycStatus && (
            <View
              style={[
                ...
                {
                  backgroundColor:
                    item.kycStatus === "approved" ? "#ECFDF5" : "#FEF2F2",
                },
              ]}
            >
              <Text
  ...
  return (
    <SafeAreaView style={commonStyles.safe}>
      ...
      <FlatList
        data={filtered}
        keyExtractor={(item) =>
          item.id ?? item.borrowerId ?? Math.random().toString()
        }

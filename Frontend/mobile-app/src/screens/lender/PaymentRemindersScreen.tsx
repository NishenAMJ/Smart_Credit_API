...
export default function PaymentRemindersScreen({ navigation }: any) {
  ...
  const filtered = reminders.filter(r => (r.status ?? 'pending') === activeTab);
  ...
  return (
    <SafeAreaView style={commonStyles.safe}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {filtered.length === 0 ? (
          <AlertBanner type="info" title="No Reminders" message={`No ${activeTab} reminders`} />
        ) : (
          filtered.map((reminder) => (
            <View key={reminder.id} style={commonStyles.card}>
              <View style={commonStyles.rowSpaceBetween}>
                <View style={{ flex: 1 }}>
                  <Text style={commonStyles.sectionTitle}>{reminder.borrowerName ?? 'Unknown'}</Text>
                  <Text style={commonStyles.textSecondary}>{reminder.loanId ?? reminder.id}</Text>
                </View>
                <Feather name={getStatusIcon(reminder.status ?? 'pending')} size={20} color={COLORS.primary} />
              </View>
              ...
              {(reminder.status ?? 'pending') === 'pending' && (
                <TouchableOpacity
                  ...
                  onPress={() => Alert.alert('Reminder', `Reminder sent to ${reminder.borrowerName ?? 'borrower'}`)}
                >
...
const styles = StyleSheet.create({
  ...
  tab: {
    ...
    borderRadius: 6, borderWidth: 1, borderColor: COLORS.border, alignItems: 'center',
  },

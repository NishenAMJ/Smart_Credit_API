...
// ── Auth (same pattern as advertisement.service.ts) ───────────────────────────
...
export const DashboardService = {
   ...
   */
  getBorrowers: async (pageSize = 20, cursor?: string) => {
    const lenderId = getLenderId();
...
export const LenderProfileService = {
   ...
   */
  updateProfile: async (payload: UpdateProfilePayload): Promise<LenderProfile> => {
    const lenderId = getLenderId();
  ...
  changePassword: async (_payload: ChangePasswordPayload): Promise<void> => {
    throw new Error('changePassword endpoint not yet implemented on the backend.');
  },
};
...
export const LoanOffersService = {
   ...
   */
  updateOffer: async (offerId: string, data: {
    minAmount?: number;
    maxAmount?: number;
    interestRate?: number;
    tenureMonths?: number;
    ...
    const lenderId = getLenderId();
    return api.patch(`/lender-mobile/offers/${offerId}?lenderId=${lenderId}`, data);
  },
};
